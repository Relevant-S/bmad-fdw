...
# Past this point the feature is development's, not the BA's. It is the discriminator between a
# change that reopens a spec and one that has to ship beside it.
TERMINAL = {"handed-off", "shipped"}

CHANGE_ROUTES = ["in-flight", "delivered"]
CHANGE_STATUS = ["open", "absorbed", "scheduled", "delivered", "dropped"]
CHANGE_OPEN = "open"
DESIGN_IMPACT = ["true", "false", "unknown"]
...
def next_change_id(fid: str, record: dict[str, Any]) -> str:
    """The next free change id, derived from the highest in use for the same reason question ids
    are: callers may supply their own, so the list carries gaps and length-plus-one collides."""
    used = [
        int(match.group(1))
        for change in record.get("changes", [])
        if (match := re.match(rf"^{re.escape(fid)}-C-(\d+)$", str(change.get("id", ""))))
    ]
    return f"{fid}-C-{max(used, default=0) + 1:02d}"


def change_record(fid: str, change: dict[str, Any], stamp: str, source: str,
                  record: dict[str, Any], entry: dict[str, Any],
                  requirements_digest: str = "") -> dict[str, Any]:
    """One change record, in the shape the contract declares. The single mint site, for the same
    reason question_record is: fifteen keys written in two places drift apart within a month.

    `route` and `status_when_raised` are frozen here and never recomputed. A change raised while
    the spec was still the BA's is work the BA agreed to absorb; letting a later `feature-set
    --status handed-off` silently reclassify it as somebody else's would change the commitment
    underneath them."""
    design = change.get("design_invalidated", "unknown")
    if isinstance(design, bool):
        design = "true" if design else "false"
    return {
        "id": change.get("id") or next_change_id(fid, record),
        "text": change["text"],
        "anchor": change.get("anchor") or None,
        "quote": change.get("quote") or None,
        "criticality": change.get("criticality", "critical"),
        "route": "delivered" if entry.get("status") in TERMINAL else "in-flight",
        "status": CHANGE_OPEN,
        "design_invalidated": str(design),
        "status_when_raised": entry.get("status", ""),
...
def empty_change_counts() -> dict[str, int]:
    return {"in-flight": 0, "delivered": 0}


def requirements_digest(spec_text: str) -> str:
    """A hash of the Requirements section only. `absorb` compares it against the value frozen when
    the change was raised, so 'I absorbed it' has to mean the spec now says something different."""
    body = re.split(r"^##\s+", spec_text, flags=re.M)
    for chunk in body:
...
def render_changes_md(feature: dict[str, Any], record: dict[str, Any]) -> str:
    """changes.md is a reading surface, regenerated wholesale on every write.

    questions.md is derived too, but fdw-consistency regenerates it on demand. This one is
    feature-local and is read by a BA mid-absorption, so it must never be stale between two
    commands -- which means the writer regenerates it, not a later rollup."""
    lines = [
        f"# {feature['title']} — Change records",
        "",
        "Derived — regenerated by fdw_state.py on every change write. Do not hand-edit;"
        " the ledger in feature.json is the record.",
        "",
    ]
    changes = record.get("changes", [])
    if not changes:
        lines += ["_No change records._", ""]
    for change in sorted(changes, key=lambda c: (c.get("status") != CHANGE_OPEN, c.get("id", ""))):
        lines.append(f"## {change['id']} — {change.get('status', CHANGE_OPEN)}")
        lines.append("")
        lines.append(f"- what: {change.get('text', '')}")
        lines.append(f"- route: {change.get('route', '')} "
                     f"(raised while '{change.get('status_when_raised', '')}')")
        lines.append(f"- criticality: {change.get('criticality', '')}")
        lines.append(f"- design invalidated: {change.get('design_invalidated', 'unknown')}")
        if change.get("anchor"):
            lines.append(f"- anchor: `{change['anchor']}`")
        if change.get("quote"):
            lines.append(f"- quote: > {change['quote']}")
        lines.append(f"- raised: {change.get('raised', '')} by {change.get('raised_by', '')}")
        if change.get("note"):
            lines.append(f"- note: {change['note']}")
        if change.get("status") != CHANGE_OPEN:
            lines.append(f"- resolution: {change.get('resolved', '')} — {change.get('resolution', '')}")
            if change.get("absorbed_by"):
                lines.append(f"- absorbed by: {', '.join(change['absorbed_by'])}")
            if change.get("scheduled_to"):
                lines.append(f"- scheduled to: {change['scheduled_to']}")
            if change.get("delivered_in"):
                lines.append(f"- delivered in: {change['delivered_in']}")
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"
...
def _open_changes(root: Path, registry: dict[str, Any]) -> list[dict[str, Any]]:
    out = []
    for feature in registry.get("features", []):
        record = read_json(_feature_dir(root, feature) / "feature.json", {})
        for change in record.get("changes", []):
            if change.get("status", CHANGE_OPEN) == CHANGE_OPEN:
                out.append({**change, "feature_id": feature["id"], "feature_title": feature["title"]})
    return out


def _count_open_changes(record: dict[str, Any]) -> dict[str, int]:
    counts = empty_change_counts()
    for change in record.get("changes", []):
        if change.get("status", CHANGE_OPEN) == CHANGE_OPEN:
            counts[change.get("route", "in-flight")] = counts.get(change.get("route", "in-flight"), 0) + 1
    return counts


def _sync_changed_flag(entry: dict[str, Any], record: dict[str, Any]) -> None:
    """The `changed` flag is set iff an open change exists. Keeping it in one place is what stops
    the old defect where closing a record left the flag on forever."""
    has_open = any(c.get("status", CHANGE_OPEN) == CHANGE_OPEN for c in record.get("changes", []))
    for holder in (entry, record):
        flags = holder.setdefault("flags", [])
        if has_open and "changed" not in flags:
            flags.append("changed")
        elif not has_open and "changed" in flags:
            flags.remove("changed")
        ...
        features.append(
            {
                ...
                # Which of the two change paths this feature is on, so the plan can be shaped
                # before validate-plan refuses rather than after.
                "route": "delivered" if feature["status"] in TERMINAL else "in-flight",
                "open_changes": feature.get("open_changes", empty_change_counts()),
            }
        )
    ...
    emit(
        {
            ...
            "open_changes": _open_changes(root, registry),
            "glossary": glossary,
            "ingested_sources": [
...
def validate_plan(root: Path, plan: dict[str, Any]) -> list[str]:
    ...
    for i, feature in enumerate(plan.get("new_features", [])):
        ...
        elif _phase_status(root, phase) == "closed":
            errors.append(
                f"{where}: phase '{phase}' is closed. Its blocker count and PRD are a record of what "
                f"was handed off, and a feature added now would not be in either. Put it in the "
                f"open phase.")
...
def cmd_apply_plan(args: argparse.Namespace) -> None:
    ...
    stamp = today()
    delta: dict[str, Any] = {"created": [], "merged": [], "questions_closed": [], "contradictions": [],
                             "changes": [], "deferred": []}
    ...
    # Only one of the two, never both. Raising both double-counted a single fact in two ledgers
    # with two different gates, so absorbing the change still left approval refusing on a question
    # about it — and the change never counted toward the phase's own blocker metric either.
    for contradiction in plan.get("contradictions", []):
        ...
        if locked:
            spec = fdir / "spec.md"
            minted = change_record(
                fid, contradiction, stamp, source_id, record, entry,
                requirements_digest(spec.read_text(encoding="utf-8") if spec.exists() else ""))
            record.setdefault("changes", []).append(minted)
            counts = entry.setdefault("open_changes", empty_change_counts())
            counts[minted["route"]] = counts.get(minted["route"], 0) + 1
            _sync_changed_flag(entry, record)
            _write_atomic(fdir / "changes.md", render_changes_md(entry, record))
            delta["changes"].append({"id": fid, "change_id": minted["id"], "route": minted["route"],
                                     "design_invalidated": minted["design_invalidated"]})
            append_md(
                paths["decisions"],
                f"- {stamp} · event · {minted['id']} raised ({minted['route']}, design invalidated: "
                f"{minted['design_invalidated']}): {minted['text']} · source: {source_id}",
            )
        else:
            record.setdefault("questions", []).append(
...
def cmd_change_add(args: argparse.Namespace) -> None:
    """Raise a change against a feature whose spec is already approved.

    The mint trigger is SPEC_LOCKED, deliberately: below it there is no approved spec to change,
    so a new request is either an edit to a draft nobody has committed to or a question. The
    route is decided here from the feature's status and frozen onto the record."""
    root = Path(args.root).resolve()
    paths = store(root)
    registry = read_json(paths["registry"], empty_registry())
    ...
    fid = args.id or (batch or {}).get("feature")
    source = args.source or (batch or {}).get("source")
    incoming = (batch or {}).get("changes") if batch else [
        {"id": args.change_id, "text": args.text, "anchor": args.anchor, "quote": args.quote,
         "criticality": args.criticality, "design_invalidated": args.design_invalidated,
         "note": args.note}
    ]
    if not fid:
        die(["No feature id. Pass --id, or put 'feature' in the batch file."])
    if not source:
        die([f"{fid}: --source is required — where did this change come from? On a fixed-price "
             f"engagement that is the difference between absorbing a change and billing it."])
    if not incoming:
        die([f"{fid}: nothing to add."])
    ...
    if entry is None:
        die([f"No feature '{fid}'. Known: {[f['id'] for f in registry.get('features', [])]}"])
    if entry["status"] not in SPEC_LOCKED:
        die([f"{fid} is '{entry['status']}' — there is no approved spec to change. Edit the draft, "
             f"or raise it with question-add if somebody has to answer something first."],
            feature=fid, status=entry["status"])
    fdir = _feature_dir(root, entry)
    record = read_json(fdir / "feature.json")
    if record is None:
        die([f"{fid}: no feature.json at {fdir}. Run validate — the registry and the folders disagree."])
    ...
    spec = fdir / "spec.md"
    digest = requirements_digest(spec.read_text(encoding="utf-8") if spec.exists() else "")
    ingested = bool(re.match(r"^\d{4}-\d{2}-\d{2}-", str(source)))

    errors: list[str] = []
    known = {c.get("id"): c for c in record.get("changes", [])}
    open_text = {_normalise(c.get("text", "")): c for c in record.get("changes", [])
                 if c.get("status", CHANGE_OPEN) == CHANGE_OPEN}
    done_text = {_normalise(c.get("text", "")): c for c in record.get("changes", [])
                 if c.get("status", CHANGE_OPEN) != CHANGE_OPEN}
    plan: list[dict[str, Any]] = []
    already: list[str] = []
    warnings: list[str] = []

    for i, change in enumerate(incoming):
        where = f"{fid}.changes[{i}]"
        text = str(change.get("text", "")).strip()
        criticality = change.get("criticality") or "critical"
        design = change.get("design_invalidated", "unknown")
        if isinstance(design, bool):
            design = "true" if design else "false"
        design = str(design)
        ...
        quote = (change.get("quote") or "").strip() or None
        if not text:
            errors.append(f"{where}: missing 'text'.")
        if criticality not in CRITICALITY:
            errors.append(f"{where}: criticality must be one of {CRITICALITY}.")
        if design not in DESIGN_IMPACT:
            errors.append(f"{where}: design_invalidated must be one of {DESIGN_IMPACT}.")
        if ingested and not (anchor and quote):
            errors.append(
                f"{where}: --source names an ingested document, so anchor and quote are required. "
                f"Evidence from a source is anchored or it is hearsay.")
        if anchor and not ANCHOR_RE.match(anchor):
            errors.append(f"{where}: '{anchor}' is not a valid anchor.")
        if cid is not None:
            if not re.fullmatch(rf"{re.escape(fid)}-C-\d{{2,}}", str(cid)):
                errors.append(f"{where}: '{cid}' is not an id for {fid}. Shape is {fid}-C-NN.")
            elif cid in known:
                if _normalise(known[cid].get("text", "")) == _normalise(text):
                    already.append(cid)
                    continue
                errors.append(
                    f"{where}: {cid} already exists with different wording. Ids never change "
                    f"meaning — close it and raise a new one.")
                continue
        twin = open_text.get(_normalise(text))
        if twin and not args.allow_duplicate:
            errors.append(
                f"{where}: {twin['id']} is already open with the same wording. Pass "
                f"--allow-duplicate if these really are two changes.")
            continue
        settled = done_text.get(_normalise(text))
        if settled:
            warnings.append(
                f"{settled['id']} was already closed with the same wording on "
                f"{settled.get('resolved', '?')}. Worth a look before raising it again.")
        plan.append({"id": cid, "text": text, "anchor": anchor, "quote": quote,
                     "criticality": criticality, "design_invalidated": design,
    ...
    for change in plan:
        minted = change_record(fid, change, stamp, source, record, entry, digest)
        record.setdefault("changes", []).append(minted)
        counts = entry.setdefault("open_changes", empty_change_counts())
        counts[minted["route"]] = counts.get(minted["route"], 0) + 1
        added.append(minted)
    ...
    if added:
        ...
        for minted in added:
            append_md(
                paths["decisions"],
                f"- {stamp} · event · {minted['id']} raised ({minted['route']}, design "
                f"invalidated: {minted['design_invalidated']}): {minted['text']} · source: {source}",
            )

    routes = {m["route"] for m in added}
    if "delivered" in routes:
        warnings.append(
            f"{fid} is '{entry['status']}' — this change is against delivered work. It does not "
            f"reopen the spec: run fdw-handoff build-brief to send it to development on its own.")
    if "in-flight" in routes:
        warnings.append(
            f"{fid} is '{entry['status']}' — approval and bundling both refuse while this is open. "
            f"Run fdw-elaborate revise to absorb it.")

    emit({"feature": fid, "added": [m["id"] for m in added],
          "routes": {m["id"]: m["route"] for m in added},
          "already_present": already, "open_changes": entry.get("open_changes"),
          "changes_file": str((fdir / "changes.md").relative_to(root)) if added else None,
          "warnings": warnings})


def cmd_change_close(args: argparse.Namespace) -> None:
    """Close one named change record. Naming it is required: the command this replaces closed
    whichever record happened to be first in the file, which on a feature with two open changes
    silently recorded the wrong resolution against the wrong one."""
    root = Path(args.root).resolve()
    paths = store(root)
    registry = read_json(paths["registry"], empty_registry())
    ...
    entry = next((f for f in registry.get("features", []) if f["id"] == fid), None)
    if entry is None:
        die([f"'{args.change_id}' does not name a known feature. Known: "
             f"{[f['id'] for f in registry.get('features', [])]}"])
    fdir = _feature_dir(root, entry)
    record = read_json(fdir / "feature.json")
    if record is None:
        die([f"{fid}: no feature.json at {fdir}."])

    change = next((c for c in record.get("changes", []) if c.get("id") == args.change_id), None)
    if change is None:
        die([f"No change '{args.change_id}' on {fid}. Open: "
             f"{[c['id'] for c in record.get('changes', []) if c.get('status') == CHANGE_OPEN]}"])
    if change.get("status", CHANGE_OPEN) != CHANGE_OPEN:
        die([f"{args.change_id} is already '{change['status']}' "
             f"(resolved {change.get('resolved', '?')})."])

    outcome = args.outcome
    if outcome == "absorbed" and not args.absorbed_by:
        die([f"{args.change_id}: --outcome absorbed needs at least one --absorbed-by. A change "
             f"absorbed into no requirement did not change the spec, and the record has to say "
             f"which lines carry it now."])
    if outcome == "scheduled" and not args.scheduled_to:
        die([f"{args.change_id}: --outcome scheduled needs --scheduled-to. Deferring without a "
             f"destination is how a change gets lost between two phases."])
    if outcome == "delivered" and not args.delivered_in:
        die([f"{args.change_id}: --outcome delivered needs --delivered-in — the build, PR or spec "
             f"that carries it. as-built is only trustworthy if it can point at what shipped."])
    for rid in args.absorbed_by or []:
        if not re.fullmatch(rf"{re.escape(fid)}-R-\d+", rid):
            die([f"'{rid}' is not a requirement id for {fid}."])
    ...
    change.update({
        ...
        "outcome": outcome,
        "absorbed_by": args.absorbed_by or [],
        "resolved": stamp,
        "closed_by": args.by or "fdw",
    })
    if args.scheduled_to:
        change["scheduled_to"] = args.scheduled_to
    if args.delivered_in:
        change["delivered_in"] = args.delivered_in

    counts = entry.setdefault("open_changes", empty_change_counts())
    route = change.get("route", "in-flight")
    counts[route] = max(0, counts.get(route, 0) - 1)

    # A change minted before the ledger existed carried a paired question. Closing one and
    # leaving the other open would keep approval refusing on a fact already resolved.
    closed_question = None
    if change.get("question_id"):
        for question in record.get("questions", []):
            if question.get("id") == change["question_id"] and question.get("status") == "open":
                question.update({"status": "resolved", "answer": args.resolution,
                                 "answer_source": f"change {change['id']}", "resolved": stamp})
                qc = entry.setdefault("open_questions", {"critical": 0, "non-critical": 0})
                key = question.get("criticality", "critical")
                qc[key] = max(0, qc.get(key, 0) - 1)
                closed_question = question["id"]
    ...
    append_md(
        paths["decisions"],
        f"- {stamp} · decision · {change['id']} {outcome}: {args.resolution} · source: "
        f"{args.by or 'fdw'}",
    )
    emit({"feature": fid, "change": change["id"], "outcome": outcome,
          "absorbed_by": change["absorbed_by"], "closed_question": closed_question,
          "open_changes": entry.get("open_changes"), "flags": entry.get("flags", [])})
...
def _phase_status(root: Path, phase: str) -> str:
    return str((read_json(store(root)["phases"] / phase / "phase.json", {}) or {}).get("status", ""))
...
def cmd_phase_open(args: argparse.Namespace) -> None:
    ...
    if args.from_phase:
        ...
        for feature in _phase_features(registry, args.from_phase):
            ...
            # Open changes travel, and so does one parked for this phase by name — which is what
            # makes `change-close --outcome scheduled` a real destination rather than a shrug.
            carried["changes"] += [
                {"change_id": change["id"], "feature": feature["id"], "route": change.get("route"),
                 "design_invalidated": change.get("design_invalidated"),
                 "text": change.get("text", "")}
                for change in record.get("changes", [])
                if change.get("status", CHANGE_OPEN) == CHANGE_OPEN
                or change.get("scheduled_to") == args.phase
            ]
...
def cmd_phase_move(args: argparse.Namespace) -> None:
    ...
    if _phase_status(root, args.to) == "closed" and not args.force:
        die([f"Phase '{args.to}' is closed — moving {args.id} into it would change what a phase "
             f"already reported as delivered. Pass --force only if you are correcting a mistake."])
...
def cmd_phase_close(args: argparse.Namespace) -> None:
    ...
    record.update({
        ...
        # Beside the blocker count, never folded into it — that number counts critical questions
        # and earlier phases recorded theirs under that definition.
        "open_changes_at_close": sum(
            sum(_count_open_changes(read_json(_feature_dir(root, f) / "feature.json", {})).values())
            for f in members),
    })
    if args.prd_path:
...
def cmd_validate(args: argparse.Namespace) -> None:
    ...
    for feature in registry.get("features", []):
        ...
        for question in record.get("questions", []):
            ...
            if qid in seen_q:
                problems.append(f"{fid}: duplicate question id '{qid}'. Ids are minted once.")
            seen_q.add(qid)

        # Changes: the same three invariants questions get, plus the flag agreeing with the ledger.
        seen_c: set[str] = set()
        req_ids = set()
        spec = fdir / "spec.md"
        if spec.exists():
            req_ids = {m.group(1) for m in
                       re.finditer(r"\*\*\[([A-Z0-9-]+-R-\d+)\]\*\*", spec.read_text(encoding="utf-8"))}
        for change in record.get("changes", []):
            cid = str(change.get("id", ""))
            if cid in seen_c:
                problems.append(f"{fid}: duplicate change id '{cid}'. Ids are minted once.")
            seen_c.add(cid)
            if not re.fullmatch(rf"{re.escape(fid)}-C-\d{{2,}}", cid):
                problems.append(f"{fid}: change id '{cid}' is not shaped {fid}-C-NN.")
            if change.get("route") not in CHANGE_ROUTES:
                problems.append(f"{fid}: {cid} has route '{change.get('route')}', not one of {CHANGE_ROUTES}.")
            if change.get("status") not in CHANGE_STATUS:
                problems.append(f"{fid}: {cid} has status '{change.get('status')}', not one of {CHANGE_STATUS}.")
            if str(change.get("design_invalidated")) not in DESIGN_IMPACT:
                problems.append(f"{fid}: {cid} has design_invalidated "
                                f"'{change.get('design_invalidated')}', not one of {DESIGN_IMPACT}.")
            for rid in change.get("absorbed_by", []):
                if req_ids and rid not in req_ids:
                    problems.append(f"{fid}: {cid} says it was absorbed by {rid}, which is not in spec.md.")
            if change.get("question_id") and change["question_id"] not in seen_q:
                problems.append(f"{fid}: {cid} points at question {change['question_id']}, which does not exist.")
        counted_changes = _count_open_changes(record)
        if feature.get("open_changes", empty_change_counts()) != counted_changes:
            problems.append(f"{fid}: open_changes count is stale — registry "
                            f"{feature.get('open_changes')}, actual {counted_changes}.")
        has_open_change = any(c.get("status", CHANGE_OPEN) == CHANGE_OPEN for c in record.get("changes", []))
        if has_open_change != ("changed" in feature.get("flags", [])):
            problems.append(
                f"{fid}: the 'changed' flag and the change ledger disagree — flag is "
                f"{'set' if 'changed' in feature.get('flags', []) else 'clear'}, open changes: "
                f"{sum(counted_changes.values())}.")

    highest = max((int(f["id"].split("-")[1]) for f in registry.get("features", [])
                   if re.fullmatch(r"F-\d+", f.get("id", ""))), default=0)
    if registry.get("next_feature_seq", 1) <= highest:
        problems.append(f"next_feature_seq is {registry.get('next_feature_seq')} but F-{highest:03d} "
                        f"already exists. The next mint would reuse an id.")

    for phase in registry.get("phases", []):
        record_phase = read_json(paths["phases"] / phase / "phase.json", {}) or {}
        listed = set(record_phase.get("features", []))
        actual = {f["id"] for f in registry.get("features", []) if f.get("phase") == phase}
        if record_phase.get("status") == "closed" and listed != actual:
            problems.append(f"phase '{phase}' is closed but its members {sorted(listed)} disagree "
                            f"with the registry {sorted(actual)}.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    p = sub.add_parser("change-add", help="raise a change against a feature whose spec is approved")
    p.add_argument("--root", required=True)
    p.add_argument("--id", default=None, help="feature id; may also come from the batch file")
    p.add_argument("--from", dest="from_file", default=None,
                   help="batch JSON: {feature, source, changes:[{id?,text,anchor?,quote?,"
                        "criticality?,design_invalidated?,note?}]}")
    p.add_argument("--text", default=None)
    p.add_argument("--anchor", default=None, help="source anchor, required when --source is a document")
    p.add_argument("--quote", default=None, help="their words, verbatim and untranslated")
    p.add_argument("--criticality", default="critical", choices=CRITICALITY)
    p.add_argument("--design-invalidated", dest="design_invalidated", default="unknown",
                   choices=DESIGN_IMPACT,
                   help="does this change what the client already signed off on screen?")
    p.add_argument("--change-id", dest="change_id", default=None,
                   help="supply the id to make a re-run a no-op instead of a second copy")
    p.add_argument("--source", default=None, help="where it came from")
    p.add_argument("--note", default=None)
    p.add_argument("--allow-duplicate", dest="allow_duplicate", action="store_true")
    p.set_defaults(func=cmd_change_add)

    p = sub.add_parser("change-close", help="close one named change record")
    p.add_argument("--root", required=True)
    p.add_argument("--change-id", dest="change_id", required=True)
    p.add_argument("--resolution", required=True)
    p.add_argument("--outcome", default="absorbed", choices=CHANGE_STATUS[1:])
    p.add_argument("--absorbed-by", dest="absorbed_by", action="append", default=[],
                   help="requirement id that now carries this change; repeatable")
    p.add_argument("--scheduled-to", dest="scheduled_to", default=None,
                   help="phase this change was deferred to, with --outcome scheduled")
    p.add_argument("--delivered-in", dest="delivered_in", default=None,
                   help="build, PR or spec that shipped it, with --outcome delivered")
